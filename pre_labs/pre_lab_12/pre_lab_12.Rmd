...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **Answer** The columns are the same as the website, we have the date and we have the press release. But the dataframe needs some cleaning up before we could actually use it. Particularly noticeable are the dates which are not in date format. 
...
**Task** Run the following code to extract the table we want, clean up the names and change the date column to an actual date. Look at the dataframe; what other information about these releases might you want to add? **Answer** When you inspect the table you see that it also contains an href to the actual press release. If there's anything else you might be able to add to the dataframe, I think it could be the URL to the actual site of the press release. 
...
tags there are in the HTML source? **Answer** The output suggest there are 122 tags in the HTML source. 
...
tags. Describe the output: **Answer** The output shows each 'li" element and the <li>text within the elements </li>
...
**Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **Answer** The output is a dataframe of those 122 "li" elements. There's only one variable, the "value," and the value is the text listed inside the "li" element.
