...
A.) summary and colnames gives the names of my 8 columns and tells me information about each column such as how many rows it has, and for numbers it appears to give me statistical data on the numbers in the column
...
A: It is important that we make a new variable to hold the result, because we may work with that variable later, similar to how we worked with the "journalism_courses" variable in task 5. We used that and then from the filtered journalism courses we selected only the columns "id" and "title" 
...
The OR gives us 41011 rows of data. 
**Answer**
...
head(maryland_expenses)
```
I now see the date in a different format - YYYY-MM-DD. Date is also listed under the column header instead of chr. 
**Answer:**
...
In the new month column, months are written in the format YYYY-MM-DD, but all the days are the first of the month - pay day! 
...
The code is telling R to go to the general_22 data, add all the votes each candidate receive - total votes - and come up with a percentage of the total votes that Moore received. 
...
I'm actually not seeing an answer when I run the code. 
...
It's more useful, because you can easily see where Moore received the least percent of the vote. 
...
On https://stringr.tidyverse.org/reference/case.html I see that you can convert words to title case by using str_to_title, and I see you can also convert words all to lower case by using str_to_lower
...
I think the code is first looking at the maryland expenses then it is creating a new column called "state". Then it is going through the address  data line by line so that if space Maryland space is in the address than it will add MD to the state column. 
...
I think it makes sense. I think what we did was totaled the amount of expenses for each state and then stacked the data in descending order. 
...
1. For the UMD data I would filter out "NA" responses for professors to ask questions siimilar to what we asked in previous exercises. For example, which professor teaches the most classes. If I filter out NA, I can say...of the classes where professors names were listed, professor x taught the most classes. 

2.  If I only wanted to know about the term, department, and professor I would select those categories for my data analysis, eliminating other columns from my data. 

3.  Some titles in the titel column have portions of the title in all caps. I would make them all uniform by using the str_to_title mutation to mutate the titles so they are all uniform. 
