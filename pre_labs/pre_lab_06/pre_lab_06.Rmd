...
**Task** Load the four CSV population estimate files. Do they have the same column structure? **Answer** Yes, they all have 24 rows and 9 columns. The header for the last column of the 2022 data had total spelled differently, so we changed it to all caps to have that loast column head uniform across the four dataframes. 
...
**Task** Run the following code to filter on Frederick County's registration data and look at the results. What stands out to you about the changes in party registration over time? **Answer** What stands out to me about party registration in Frederick County is that in 2016 Republicans outnumbered Democrats, but in 2022, the opposite was true for registered voters' party affiliations. 
...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** There are now 24 rows. 
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer** demographic data contains the geocode of the area, the corresponding county (or city in the case of Baltimore), age in terms of citizens over 18, and a count of the "ethnicity/race" of the people per geographic area. 
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** Prince George's county has the highest 18+ black population and the highest % democrats per county 
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** Oh. Howard county has the highest 18+ asian population. Roughly 52.3 % of that county's voters are registered democrats. The voter registration varies from Prince George's in that there are roughly 20% less registered democrats in Howard County than in Prince George's. 
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Answer** the Zcta5n column has transformed into a chr datatype. 
```{r}
