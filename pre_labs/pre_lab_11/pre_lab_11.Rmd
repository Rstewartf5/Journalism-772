...
**Task** Run the following code and describe the dataframe it outputs. **Answer** when I run the below code I see a dataframe with four columns and 999 rows of Ben Cardin's releases. The columns show the dates of the press releases, the title of each of the press releases, the URLs where those press releases can be found and the text of the press release. 
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** it appears that the function counts the number of times when one of the words in the list appear. 
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** It seems like the unnest_tokens function searches the text and finds the most commonly used words and then ranks those. 
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** The resulting graph seems to make a bar chart that shows the top words in the press releases. It could be interesting... I mean many of these words are among the top words used in the English language, so it doesn't tell us much about the content of his press releases. As someone who studied Germanic languages as an undergraduate the list is interesting to me, because it demonstrates the frequency of which Germanic-orginin in English speech. The only latin loan word I could pick up in the list was the inferred "United" in the word U.S. Also, it makes me wonder if  political press releases are intentionally simple. 

I don't think there's much newsworthy however. 
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** Hello Latin-origin words! I guess both lists are interesting, but one is probably more interesting in understanding the content of his thoughts. :-) Cardin cares about health (a word that happens to be of Germanic origin ). 
...
Super interesting. Health is important to Cardin each year, but you hear some of the words that reflect the political atmosphere. 2021 we're talking about COVID, 2022 Baltimore pops out to me, and I wonder if it has something to do with the elections as democrats like to pander to the city during elections, and 2023 women come up and that could be because of political concerns surrounding the repeal of Roe v. Wade. I wonder what words other Maryland politicians are using frequently in press releases, what other democrat leaders are using and if they are similar. 
...
**Task** Run the following code and describe the results. **Answer** This is even cooler. Getting the bigrams like this you can see real themes. Cardin isn't just talking about health, he's creating press releases pertaining to public health. I was expecting women's rights to be coupled with one another, but it sounds like he's talking about human rights, which is very on brand for a democratic senator. I'm thinking if we go trigrams that American Rescue and Rescue Plan often go togehter as the American Rescue Plan. 
...
**Task** Run the following code and describe the results. **Answer** Now we get a lot more bigrams that are Maryland focused and a few very bureaucratic words. 
...
**Task** Run the following code and describe the results. **Answer** It sounds like in 2023 Cardin's press releases are really focused on policy issues. I think we would get a better sense of his issues if we had trigrams. 
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** I'm sure there are groups of very smart people who have come up with a point system to determine how frequently one word is used in a positive or negative context. Critical is an example of one that you just feel like it is not necessarily always negative. Critical Infrastructure for example could be talked about in a very aspirational way, which would feel positive, but if you're talking about being critical of someone that could be negative. 
