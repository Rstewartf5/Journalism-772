...
```{r}
voters_by_county <- read_csv("tabula_eligible_by county_GG22.csv")
...
```{r}
fentanyl_deaths <- read_csv("tabula-fetanyl_deaths.csv") |> clean_names()
...
We loaded the table with no headers, so there are weird markings as headers: x1, x2, x3, etc. Also there are a serious of periods after the names of each Maryland county in the first column. 

```{r}
fentanyl_deaths <- read_csv("tabula-fetanyl_deaths.csv", col_names = FALSE) |> clean_names()
...
I think now we will need to clean up the series of periods in the column we labeled "jurisdiction."

```{r}
fentanyl_deaths <- read_csv("tabula-fetanyl_deaths.csv", col_names = FALSE) |> 
  clean_names() |> 
