...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** Several of the grantees have very similar names - and that is just in the grantees that begin with the letter W! 
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer**  (1) In the cleaned version the "Inc" was removed from the Melwood Horticultural Training Center. (2) The Treehouse Child Advocacy Center...is written in all caps in the cleaned version, and (3) Inc has been removed from the Charles County Arts Alliance Inc in the cleaned version and the cleaned version has the organization all in caps. 
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer** The only change I see in the first 30 results is one change to Washington Adventist University. It had previously been all caps, now the name of the grantee is in title form. 
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** Reviewing the first 30 rows, the results of performing task 7 do NOT seem substanitally different from the results of performing task 6. 
